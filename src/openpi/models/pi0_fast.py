    ...
    def sample_actions(
        ...
        *,
        ...
        def _sample_one(rng_one):
            aux_accum = {
                "encoded": jnp.zeros(
                    (batch, max_decoding_steps + 1, self.PaliGemma.llm.module.width), dtype=last_logit.dtype
                ),
                "logits": jnp.zeros(
                    (batch, max_decoding_steps + 1, self.PaliGemma.llm.module.vocab_size), dtype=last_logit.dtype
                ),
                "pre_logits": jnp.zeros(
                    (batch, max_decoding_steps + 1, self.PaliGemma.llm.module.width), dtype=last_logit.dtype
                ),
            }
            ...
            def body(c):
                ...
                # sampling
                token = (
                    jax.random.categorical(rng_one, last_logit / temperature, axis=-1)
                    if temperature > 0
                    else jnp.argmax(last_logit, axis=-1)
                )
