...
class Policy(BasePolicy):
        ...
        *,
        ...
        if self._is_pytorch_model:
            ...
            self._sample_actions = nnx_utils.module_jit(
                model.sample_actions, static_argnames=("temperature", "n_action_samples")
            )
    ...
    def _infer_maybe_multi(self, obs: dict) -> dict:  # type: ignore[misc]
        ...
        for i in range(batch_size):
            ...
            outputs_transformed.append(self._output_transform(jax.tree.map(lambda x: np.asarray(x[i, ...]), outputs)))
        outputs_transformed = {
            k: jnp.stack([v[k] for v in outputs_transformed], axis=0) for k in outputs_transformed[0].keys()
        }
...
class PolicyRecorder(_base_policy.BasePolicy):
    ...
    def infer(self, obs: dict) -> dict:  # type: ignore[misc]
        ...
        if "run/run_note" in obs:
            save_name = save_name + f"--{obs['run/run_note']}"
        if "run/task_id" in obs:
            save_name = save_name + f"--task_{obs['run/task_id']}"
        if "run/episode_idx" in obs:
            save_name = save_name + f"--ep_{obs['run/episode_idx']}"
        if "run/timestep" in obs:
            save_name = save_name + f"--t_{obs['run/timestep']}"
